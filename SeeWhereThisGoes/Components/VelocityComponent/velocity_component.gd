...
func accelerate_to_target(target: Node2D, delta: float):
	...
	owner_node.rotation = direction_to_target.angle() + PI/2 # adding PI/2 because the sprite faces up normally so we're adding a quarter turn to the right for the sprite to make it face correctly with this math.  Look at the length of this comment out here!
